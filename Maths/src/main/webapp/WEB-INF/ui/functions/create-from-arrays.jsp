    ...
    <script>
        ...
        function showError(title, message) {
            ...
            if (errorTitle && errorMessage && errorModal) {
                ...
                // ✅ ФИКС: клик вне модалки
                const closeModal = (e) => { if (e.target === errorModal) closeErrorModal(); };
                errorModal.removeEventListener('click', closeModal);
                errorModal.addEventListener('click', closeModal);

                // ✅ ФИКС: ESC
                const escHandler = (e) => { if (e.key === 'Escape') closeErrorModal(); };
                document.removeEventListener('keydown', escHandler);
                document.addEventListener('keydown', escHandler);

                const modalButton = errorModal.querySelector('button');
                if (modalButton) {
                    modalButton.style.backgroundColor = document.body.classList.contains('dark-theme') ? '#c62828' : '#f44336';
                }
            } else {
                alert(title + ': ' + message);
            }
