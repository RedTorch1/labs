...
public class FunctionService {
    ...
    public List<Point> getOrComputePoints(Long functionId, double xmin, double xmax, double step) {
        List<Point> existing = pointRepository.findByFunctionIdOrderByXValueAsc(functionId);
        if (!existing.isEmpty()) {
            return existing;
        }
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            if (pointRepository.findByFunctionIdAndXValue(functionId, xb).isPresent()) {
                continue;
            }
        ...
        return pointRepository.findByFunctionIdOrderByXValueAsc(functionId);
    }
